...
const Clients = () => {
  ...
  const fetchClients = async () => {
    try {
      const res = await axios.get('https://hicap-backend-4rat.onrender.com/api/client');
      setClients(res.data || []);
      console.log(res)
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`https://hicap-backend-4rat.onrender.com/api/client/${editId}`, formData);
        Swal.fire('Updated!', 'Client updated successfully', 'success');
      } else {
        await axios.post('https://hicap-backend-4rat.onrender.com/api/client', formData);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.delete(`https://hicap-backend-4rat.onrender.com/api/client/${id}`);
